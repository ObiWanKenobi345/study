...
// обработки запроса клиентской программы
bool GetRequestFromClient(char* name, short port, sockaddr* from, int* sizefrom)
{
	try {
		...
		while (true)
		{
			if ((recvfrom(serverSocket, ibuf, 100, NULL, from, sizefrom)) == SOCKET_ERROR)
...
// подтверждения сервером  запроса  клиента на установку соединения
...
bool GetServer(char* call, short port, sockaddr* from, int lfrom) {
	try {
		...
		((SOCKADDR_IN*)from)->sin_addr.s_addr = inet_addr("192.168.1.255"); // всем 
...
void main()
{
	...
	try {
		...
		bool resServ = GetServer("Hello", 2000, (LPSOCKADDR)&clientConfig, sizeof(clientConfig));
		...
		while (true)
		{
			...
			memset(&clientConfig, 0, sizeof(clientConfig)); // заполняет num байтов блока памяти
